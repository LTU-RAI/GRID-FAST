...
bool PolygonHandler::optimizeIntersection(polygon* poly,OpeningHandler* openingList, MapHandler* map){
    ...
    for(int sideIndex=0;sideIndex<poly->openings.size();sideIndex++){
        ...
        // ROS_INFO("t5.5");
        vector<wallCell*> p;
        // ROS_INFO("%li, %li",w1.connectedtOpeningEnd.size(),w2.connectedtOpeningStart.size());
        // ROS_INFO("t5.54");
        openingList->getPointsBetweenOpenings(w1.connectedtOpeningEnd[0],false,targetOp1,true,&p);
        if(p.size()!=0) p.pop_back(); //getPointsBetweenOpenings include firs and las point remove to not get duplicated points 
        openingList->getPointsBetweenOpenings(targetOp1,true,targetOp2,false,&p);
        if(p.size()!=0) p.pop_back();
        openingList->getPointsBetweenOpenings(targetOp2,false,w2.connectedtOpeningStart[0],true,&p);
        // ROS_INFO("t5.55 %li, %i ,%i",p.size(),loopfrom,loopto);
        if(p.size()!=0) currentWall.insert(currentWall.end(),p.begin()+loopfrom,p.end()-loopto);
