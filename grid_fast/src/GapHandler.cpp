...
void GapHandler::analysis(MapHandler *map, MapTransform *transform) {
  ...
  for (int angleIndex = 0; angleIndex < numberOfDir; angleIndex++) {
    for (int row = 0; row < GapHandler::getSizeRows(angleIndex); row++) {
      for (int index = 0; index < GapHandler::getSizeGaps(angleIndex, row);
           index++) {
...
void GapHandler::analysisAtAngle(int angleIndex, MapHandler *map,
...
void GapHandler::analysisAtRow(int angleIndex, int row, MapHandler *map,
                               MapTransform *transform) {
  ...
  for (int index = 0; index < maxIndex; index++) {
    mapTransformCell mt =
        transform->getMapTransformCell(angleIndex, row, index);
    // Finde groups
    ...
    if (mapValue == 0 && index != maxIndex - 1) {
      ...
      if (sg.end - sg.start >= minGroupSize) {
        ...
        if (transform->getMapAtTransform(sg.start - 1, row, angleIndex, map) ==
                MAP_OCCUPIED ||
            transform->getMapAtTransform(sg.end + 1, row, angleIndex, map) ==
                MAP_OCCUPIED) {
          value = MAP_OCCUPIED;
        } else {
          value = MAP_UNKNOWN;
...
void GapHandler::fillGapAtMap(MapHandler *map, MapTransform *transform,
                              int angleIndex, scanGroup *gap) {
  int value = MAP_UNOCCUPIED;
  if (!gap->traversable) {
    if (transform->getMapAtTransform(gap->start - 1, gap->row, angleIndex,
                                     map) == MAP_OCCUPIED ||
        transform->getMapAtTransform(gap->end + 1, gap->row, angleIndex, map) ==
            MAP_OCCUPIED) {
      value = MAP_OCCUPIED;
    } else {
      value = MAP_UNKNOWN;
  ...
  for (int x = gap->start; x <= gap->end; x++) {
    GapHandler::toBeFilterdPoints.push_back(
        transform->getMapIndexTransform(x, gap->row, angleIndex));
    GapHandler::toBeFilterdValues.push_back(value);
  }
}
...
scanGroup *GapHandler::get(int angleIndex, int row, int index) {
  if (angleIndex > numberOfDir)
    return NULL;
  if (row >= GapHandler::gaps[angleIndex].size())
    return NULL;
  if (index >= GapHandler::gaps[angleIndex][row].size())
    return NULL;
...
bool GapHandler::updateGap(scanGroup *gap) {
  ...
  for (int sides = 0; sides < 2; sides++) {
    ...
    if (row < 0 || row >= GapHandler::gaps[gap->angle].size())
      continue;

    for (int index = 0; index < GapHandler::gaps[gap->angle][row].size();
         index++) {
      // Skip nontraversable gaps
      if (!GapHandler::gaps[gap->angle][row][index]->traversable)
        continue;
      if (!GapHandler::checkForOverlap(GapHandler::gaps[gap->angle][row][index],
                                       gap))
        break;
    }
  }
...
void GapHandler::remove(scanGroup *gap) {
  ...
  for (int i = 0; i < GapHandler::gaps[gap->angle][gap->row].size(); i++) {
    if (GapHandler::gaps[gap->angle][gap->row][i] != gap)
      continue;
...
void GapHandler::clear() {
  for (int angleIndex = 0; angleIndex < GapHandler::gaps.size(); angleIndex++) {
    for (int row = 0; row < GapHandler::gaps[angleIndex].size(); row++) {
      for (int index = 0; index < GapHandler::gaps[angleIndex][row].size();
           index++) {
...
void GapHandler::cleanConnections(scanGroup *gap) {
  for (int i1 = 0; i1 < gap->prevGroup.size(); i1++) {
    ...
    for (int i2 = 0; i2 < conectedGap->nextGroup.size(); i2++) {
      if (conectedGap->nextGroup[i2] != gap)
        continue;
  ...
  for (int i1 = 0; i1 < gap->nextGroup.size(); i1++) {
    ...
    for (int i2 = 0; i2 < conectedGap->prevGroup.size(); i2++) {
      if (conectedGap->prevGroup[i2] != gap)
        continue;
...
bool GapHandler::checkForOverlap(scanGroup *gap1, scanGroup *gap2) {
  ...
  if (startGap1 > endGap2 || endGap1 < startGap2)
    return true;
