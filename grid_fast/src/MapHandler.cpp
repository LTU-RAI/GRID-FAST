...
void MapHandler::updateMap(vector<int> newMap, int newSizeX, int newSizeY,
                           float newResulution, double newMapOffsetX,
                           double newMapOffsetY, double newMapOffsetZ,
                           double newMapHight) {
...
int MapHandler::getMapSizeX() { return MapHandler::mapSizeX; }

int MapHandler::getMapSizeY() { return MapHandler::mapSizeY; }

double MapHandler::getMapOffsetX() { return MapHandler::mapOffsetX; }

double MapHandler::getMapOffsetY() { return MapHandler::mapOffsetY; }

double MapHandler::getMapOffsetZ() { return MapHandler::mapOffsetZ; }

double MapHandler::getMapHight() { return MapHandler::mapHight; }

float MapHandler::getMapResolution() { return MapHandler::resulution; }

int MapHandler::getMap(int x, int y) {
  if (x < 0 || x >= MapHandler::mapSizeX)
    return -1;
  if (y < 0 || y >= MapHandler::mapSizeY)
    return -1;
...
vector<int> MapHandler::getMapComplete() { return map; }
...
ant_data MapHandler::ant_step(ant_data oldStep, bool clockwise = true) {
  ...
  if (oldStep.dir.x == 0 && oldStep.dir.y == 0) {
    ...
    int c = 0;
    int l = 1;
    while (MapHandler::getMap(start.x + step.dir.x * l,
                              start.y + step.dir.y * l) == MAP_UNOCCUPIED) {
  ...
  for (int d = 0; d < 4; d++) {
    bool check = false;
    if (MapHandler::getMap(start.x + step.dir.x, start.y + step.dir.y) ==
        MAP_UNOCCUPIED) {
      point_int dir = step.dir;
      if (dir.x * dir.y != 0 && false) {
        dir = MapHandler::rotate_dir(dir, clockwise);
        if (MapHandler::getMap(start.x + dir.x, start.y + dir.y) ==
            MAP_UNOCCUPIED)
          check = true;
      } else {
        check = true;
    ...
    if (check) {
      ...
      for (int e = 0; e < 8; e++) {
        ...
        if (MapHandler::getMap(step.end.x + dir.x, step.end.y + dir.y) ==
            MAP_UNKNOWN) {
          step.emty_cell = true;
...
int MapHandler::checkForWallRay(point_int p1, point_int p2) {
  ...
  point normdVop = {((p2.x - p1.x) / (opLenght)) * l,
                    ((p2.y - p1.y) / (opLenght)) * l};
  for (int wallScan = 0; wallScan < (int)(opLenght / l); wallScan++) {
    point_int pp = {p1.x + (int)(std::round(normdVop.x * wallScan)),
                    p1.y + (int)(std::round(normdVop.y * wallScan))};
