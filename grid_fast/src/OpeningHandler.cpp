...
void OpeningHandler::updateDetections(MapHandler *map, MapTransform *transform,
                                      GapHandler *gaps) {
  ...
  for (int angleIndex = 0; angleIndex < numberOfDir; angleIndex++) {
    for (int row = 0; row < gaps->getSizeRows(angleIndex); row++) {
      for (int index = 0; index < gaps->getSizeGaps(angleIndex, row); index++) {
        OpeningHandler::checkForDetection(angleIndex, row, index, map,
                                          transform, gaps);
      }
    }
  }
  ...
  for (int i = 0; i < OpeningHandler::detectionList.size(); i++) {
    if (OpeningHandler::detectionList[i].connectedWallStart != NULL &&
        OpeningHandler::detectionList[i].connectedWallEnd != NULL)
      continue;
    OpeningHandler::detectionList.erase(OpeningHandler::detectionList.begin() +
                                        i);
    i--;
  }
}

void OpeningHandler::checkForDetection(int angleIndex, int row, int index,
                                       MapHandler *map, MapTransform *transform,
                                       GapHandler *gaps) {
  scanGroup *gap = gaps->get(angleIndex, row, index);
  for (int direction = 0; direction < 2; direction++) {
    vector<scanGroup *> connectedGaps =
        direction ? gap->prevGroup : gap->nextGroup;
    if (connectedGaps.size() < 2)
      continue;
    ...
    for (int i = 0; i < connectedGaps.size(); i++) {
      ...
      if (dist(newOp.start, newOp.end) < minGroupSize)
        continue;
      OpeningHandler::detectionMap[newOp.start.x][newOp.start.y].push_back(
          OpeningHandler::detectionList.size());
      OpeningHandler::detectionMap[newOp.end.x][newOp.end.y].push_back(
          OpeningHandler::detectionList.size());
...
opening OpeningHandler::rotateOpening(opening op, int angleIndex,
...
void OpeningHandler::correctOpening(opening *op, MapHandler *map) {
  ...
  for (int k = 0; k < 2; k++) {
    ...
    while (map->getMap(p1->x, p1->y) != 0 ||
           map->getMap(p1->x + 1, p1->y) != 0 &&
               map->getMap(p1->x - 1, p1->y) != 0 &&
               map->getMap(p1->x, p1->y + 1) != 0 &&
               map->getMap(p1->x, p1->y - 1) != 0) {
    ...
    for (int d1 = 0; d1 < 8 && !done; d1++) {
      ...
      for (int d2 = 0; d2 < 4; d2++) {
        if (map->getMap(p1->x + dir1.x + dir2.x, p1->y + dir1.y + dir2.y) !=
            MAP_UNOCCUPIED) {
      ...
      if (d1 == 7 && !done)
        *p2 = *p1; // make the opening smaller then minsize so it will be
                   // removed
    }
  }
}
...
void OpeningHandler::getWalls(MapHandler *map) {
  ...
  for (int detectIndex = 0; detectIndex < detectionList.size(); detectIndex++) {
    if (OpeningHandler::detectionList[detectIndex].label == 10)
      continue;
    for (int side = 0; side < 2; side++) {
      if (side &&
              OpeningHandler::detectionList[detectIndex].connectedWallStart !=
                  NULL ||
          !side &&
              OpeningHandler::detectionList[detectIndex].connectedWallEnd !=
                  NULL)
        continue;
      point_int p = side ? detectionList[detectIndex].start
                         : detectionList[detectIndex].end;
  ...
  for (int i = OpeningHandler::detectionList.size() - 1; i >= 0; i--) {
    if (OpeningHandler::detectionList[i].label != 10)
      continue;
...
void OpeningHandler::getAndFilterWall(MapHandler *map, point_int start,
                                      vector<point_int> *fillPoints) {
  ant_data step;
  step.end = start;
  ...
  for (int s = 0; s < 20000000; s++) {
    ...
    if (firstEmpty.x != -1) {
      fronterToRemove.insert(fronterToRemove.end(),
                             step.fronterPositions.begin(),
                             step.fronterPositions.end());
    ...
    for (int i = 0; i < detectionMap[step.end.x][step.end.y].size(); i++) {
      ...
      if (detectionList[detectIndex].start == step.end) {
        ...
        if (firstEmpty.x != -1 &&
            (detectionList[detectIndex].sideToMove == 3 ||
             detectionList[detectIndex].sideToMove == 2)) {
      ...
      if (detectionList[detectIndex].end == step.end) {
        ...
        if (firstEmpty.x != -1 &&
            (detectionList[detectIndex].sideToMove == 3 ||
             detectionList[detectIndex].sideToMove == 1)) {
          detectionsToRemove.push_back(detectIndex);
        }
      }
    ...
    if (step.end == start) {
      ...
      if (test == newWall->wall[0]->position)
        break;
    }
  }
...
void OpeningHandler::update(MapHandler *map) {
  ...
  for (int i = 0; i < OpeningHandler::size(); i++) {
    if (OpeningHandler::openingList[i]->label > 10)
      continue;
    if (OpeningHandler::openingList[i]->label == 4)
      continue;
    for (int j = 0; j < OpeningHandler::size(); j++) {
      if (j == i)
        continue;
      if (OpeningHandler::openingList[j]->label > 10)
        continue;
  ...
  for (int i = 0; i < OpeningHandler::size(); i++) {
    ...
    if (OpeningHandler::openingList[i]->label == 4)
      continue;
    if (dist(OpeningHandler::openingList[i]->start(),
             OpeningHandler::openingList[i]->end()) < minGroupSize) {
    ...
    for (int j = 0; j < OpeningHandler::size(); j++) {
      ...
      if (!OpeningHandler::intersectOpenings(OpeningHandler::openingList[i],
        ...
        continue;
      if (dist(OpeningHandler::openingList[i]->start(),
               OpeningHandler::openingList[i]->end()) <
                  dist(OpeningHandler::openingList[j]->start(),
                       OpeningHandler::openingList[j]->end()) &&
              OpeningHandler::openingList[j]->label != 4 ||
          OpeningHandler::openingList[i]->label == 4) {
  ...
  for (int i = 0; i < OpeningHandler::size(); i++) {
    ...
    if (OpeningHandler::openingList[i]->label == 4)
      continue;
    if (!check_unnecessary_openings(OpeningHandler::openingList[i], map))
      continue;
...
void OpeningHandler::findOpenings(MapHandler *map) {
  ...
  for (int i = 0; i < dList.size(); i++) {
    ...
    opening op;
    int selectedIndex =
        checkForPotensialOpening(map, i, &dList, &ignorList, &op);
    ...
    int testIndex = checkForPotensialOpening(map, selectedIndex, &dList,
                                             &emtyIgnorList, NULL, true);
    if (testIndex != i) {
...
int OpeningHandler::checkForPotensialOpening(MapHandler *map, int targetIndex,
                                             vector<opening> *dList,
                                             vector<int> *ignorList,
                                             opening *op, bool returnFirst) {
  opening bestNewOp;
  double bestScore = -1;
  int selectedIndex = -1;
  ...
  for (int index = 0; index < dList->size(); index++) {
    if (checkForComonElement(*ignorList, index))
      continue;
    if (targetIndex == index)
      continue;
    if (dList->at(index).sideToMove == 3)
      continue;
    if (dList->at(targetIndex).sideToMove == dList->at(index).sideToMove)
      continue;
    if (!OpeningHandler::intersectOpenings(&dList->at(targetIndex),
                                           &dList->at(index)))
      continue;
    point_int interPoint = OpeningHandler::findIntersectionPoint(
        dList->at(targetIndex), dList->at(index));
    ...
    if (score < minGroupSize)
      continue;

    if (bestScore < 0 || score < bestScore) {
      if ((newOp.end.x - newOp.start.x) * (interPoint.y - newOp.start.y) -
              (newOp.end.y - newOp.start.y) * (interPoint.x - newOp.start.x) >
          0)
        continue;
      if (OpeningHandler::checkForWall(&newOp, map))
        continue;
      // if(OpeningHandler::check_unnecessary_openings(newOp,map)) continue;
      selectedIndex = index;
      bestNewOp = newOp;
      bestScore = score;
      if (returnFirst && index != 0)
        break;
    }
  }
  if (op != NULL)
...
point_int OpeningHandler::findIntersectionPoint(opening o1, opening o2) {
  ...
  // Find intersection
  point p;
  if (std::isinf(m1)) { // Line 1 is vertical, so intersection point has
                        // x-coordinate of line 1
    p = {b1, m2 * b1 + b2};
  } else if (std::isinf(m2)) { // Line 2 is vertical, so intersection point has
                               // x-coordinate of line 2
    p = {b2, m1 * b2 + b1};
  } else {
...
bool OpeningHandler::check_unnecessary_openings(openingDetection *o,
                                                MapHandler *map) {
  if (o->getConnection(true)->parent != o->getConnection(false)->parent)
    return false;

  int maxSteps = (dist(o->getConnection(true)->position,
                       o->getConnection(false)->position) *
                  2);
  if (double(o->getConnection(true)->parent->getDistans(
          o->getConnection(true)->index, o->getConnection(false)->index)) <
      maxSteps)
    return true;
...
void OpeningHandler::fixOverlapingPoints(MapHandler *map) {
  ...
  for (int index = 0; index < OpeningHandler::size(); index++) {
    for (int sids = 0; sids < 2; sids++) {
      ...
      if (OpeningHandler::checkForOpenings(targetWall->wall[targetWallIndex]) <=
          1)
        continue;
      bool cw = true;
      int wMove = 1;
      for (int s = 0; s < objectFilterMaxStep; s++) {
        ...
        if (OpeningHandler::checkForOpenings(w) == 0) {
          ...
          for (int i = 0; i < w2->connectedtOpeningStart.size(); i++) {
            ...
            double ang = angleBetweenLines(op->end(), op->start(), w->position,
                                           w2->position);
          ...
          for (int i = 0; i < w2->connectedtOpeningEnd.size(); i++) {
            ...
            double ang = angleBetweenLines(op->start(), op->end(), w->position,
                                           w2->position);
            if (ang < bestAngle || bestAngle < 0) {
              opToMove = op;
              moveStart = false;
...
int OpeningHandler::checkForOpenings(wallCell *cell) {
  return cell->connectedtOpeningEnd.size() +
         cell->connectedtOpeningStart.size();
}

bool OpeningHandler::checkForWall(opening *o, MapHandler *map) {
  vector<point_int> p = OpeningHandler::generateOpeningPoints(o);
  for (int i = 0; i < p.size(); i++) {
    if (map->getMap(p[i].x, p[i].y) == MAP_OCCUPIED)
      return true;
  }
  return false;
}

int OpeningHandler::size() { return OpeningHandler::openingList.size(); }
...
openingDetection *OpeningHandler::get(int index) {
  if (index > OpeningHandler::openingList.size())
    return NULL;
...
opening *OpeningHandler::getDetection(int index) {
  if (index > OpeningHandler::detectionList.size())
    return NULL;
...
openingDetection *OpeningHandler::add(opening newOp) {
  if (newOp.start.x == -1 || newOp.start.y == -1)
    return NULL;
  if (newOp.connectedWallEnd == NULL || newOp.connectedWallStart == NULL)
    return NULL;
...
void OpeningHandler::remove(openingDetection *op) {
  ...
  for (int i = 0; i < OpeningHandler::openingList.size(); i++) {
    if (OpeningHandler::openingList[i] != op)
      continue;
...
void OpeningHandler::disable(openingDetection *op, int label) {
  op->label = label;
  if (show_removed_openings)
    OpeningHandler::openingDebug.push_back(op->getOpening());
...
bool OpeningHandler::intersectOpenings(openingDetection *o1,
                                       openingDetection *o2) {
  ...
  if (!(p1Min.x <= p2Max.x && p1Max.x >= p2Min.x && p1Min.y <= p2Max.y &&
        p1Max.y >= p2Min.y))
    return false;
  if (o1->end() == o2->end())
    return true;
  ...
  // point n1={((o1->end().x-o1->start().x)/l1)*l,
  // ((o1->end().y-o1->start().y)/l1)*l}; point
  // n2={((o2->end().x-o2->start().x)/l2)*l,
  // ((o2->end().y-o2->start().y)/l2)*l}; point
  // A={o1->start().x-n1.x,o1->start().y-n1.y},
  // B={o1->end().x+n1.x,o1->end().y+n1.y}; point
  // C={o2->start().x-n2.x,o2->start().y-n2.y},
  // D={o2->end().x+n2.x,o2->end().y+n2.y}; return
  // OpeningHandler::ccw(A,C,D)!=OpeningHandler::ccw(B,C,D) &&
...
bool OpeningHandler::intersectOpenings(opening *o1, opening *o2) {
  ...
  double l2 = dist(o2->start, o2->end);
  point n1 = {((o1->end.x - o1->start.x) / l1) * l,
              ((o1->end.y - o1->start.y) / l1) * l};
  point n2 = {((o2->end.x - o2->start.x) / l2) * l,
              ((o2->end.y - o2->start.y) / l2) * l};
  point A = {o1->start.x - n1.x, o1->start.y - n1.y},
        B = {o1->end.x + n1.x, o1->end.y + n1.y};
  point C = {o2->start.x - n2.x, o2->start.y - n2.y},
        D = {o2->end.x + n2.x, o2->end.y + n2.y};
...
int OpeningHandler::fixOverlap(openingDetection *o1, openingDetection *o2,
  ...
  for (int sides = 0; sides < 2; sides++) {
    for (int dir = 0; dir < 2; dir++) {
      ...
      if (connectedWall1->parent != connectedWall2->parent)
        continue;
      int connectedIndex1 = connectedWall1->index;
      int connectedIndex2 = connectedWall2->index;
      int connectL =
          connectedWall1->parent->getDistans(connectedIndex1, connectedIndex2);
      if (conection_lenght[sides] != -1 && connectL >= conection_lenght[sides])
        continue;
      conection_lenght[sides] = connectL;
      moveEndO2[sides] = dir;
    }
  }
  // If overlapping opening doesnt share a wall with o then keep the shortest
  // opening.
...
// retruns next wall cell on wall conected to op on 1: start 2: end. type is
// serching for a 1: start 2: end 3: both
wallCell OpeningHandler::getNextOpening(openingDetection *op, bool startSide,
                                        int type, bool cw, bool checkFirst,
                                        bool stopAtEmpty,
                                        vector<wallCell *> *pointList) {
  ...
  for (int s = 0; s < targetWall->size(); s++) {
    ...
    if (stopAtEmpty && w->emptyNeighbour)
      return *w;
    for (int i = 0; i < w->connectedtOpeningStart.size(); i++) {
      if (w->connectedtOpeningStart[i]->label < 10)
        continue;
      w->connectedtOpeningStart.erase(w->connectedtOpeningStart.begin() + i);
      i--;
    }
    for (int i = 0; i < w->connectedtOpeningEnd.size(); i++) {
      if (w->connectedtOpeningEnd[i]->label < 10)
        continue;
      w->connectedtOpeningEnd.erase(w->connectedtOpeningEnd.begin() + i);
      i--;
    }

    if ((type == 1 || type == 3) && w->connectedtOpeningStart.size() > 0)
      return *w;
    if ((type == 2 || type == 3) && w->connectedtOpeningEnd.size() > 0)
      return *w;
  }
  return *targetWall->getCell(&index);
}

vector<point_int> OpeningHandler::getPointsBetweenOpenings(
    openingDetection *o1, bool startAtStartO1, openingDetection *o2,
    bool startAtStartO2, vector<wallCell *> *wallPoints) {
  ...
  for (int s = 0; s < numberOfSteps + 1; s++) {
    if (wallPoints != NULL)
      wallPoints->push_back(o1w->getCell(&index));
...
void OpeningHandler::addCustomOpening(opening op, int id) {
  for (int i = 0; i < OpeningHandler::customOpeningList.size(); i++) {
    if (OpeningHandler::customOpeningIdList[i] != id)
      continue;
...
void OpeningHandler::removeCustomOpening(int id) {
  for (int i = 0; i < OpeningHandler::customOpeningList.size(); i++) {
    if (OpeningHandler::customOpeningIdList[i] != id)
      continue;
    OpeningHandler::customOpeningList.erase(
        OpeningHandler::customOpeningList.begin() + i);
    OpeningHandler::customOpeningIdList.erase(
        OpeningHandler::customOpeningIdList.begin() + i);
    return;
  }
}
...
int OpeningHandler::maxCustomOpeningId() {
  int maxId = -1;
  for (int i = 0; i < OpeningHandler::customOpeningList.size(); i++) {
    if (OpeningHandler::customOpeningIdList[i] <= maxId)
      continue;
